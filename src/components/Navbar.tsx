    ...
    <nav className="fixed top-0 left-0 w-full z-50 border-b border-white/10 bg-endfield-base/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="font-sans text-xl font-bold tracking-tighter text-white uppercase group-hover:text-endfield-accent transition-colors">
            {logoText}
          </span>
        </Link>
                ...
                >
                  ...
                  <button className="flex flex-col items-center group cursor-pointer focus:outline-none h-full justify-center">
                    <div className="flex items-center gap-1">
                      ...
                      {/* 下拉箭头 */}
                      <svg 
                        className={`w-3 h-3 transition-transform duration-300 ${isHovering ? 'rotate-180 text-endfield-accent' : 'text-gray-600'}`} 
                    ...
                    {(isModuleActive || isHovering) && (
                      <motion.div 
                        ...
                        className="absolute bottom-0 w-full h-[2px] bg-endfield-accent shadow-[0_0_10px_#FCEE21]"
                      />
                    )}
                  </button>
                          ...
                          {moduleItems.map((subItem) => {
                            ...
                            return (
                              <Link 
                                ...
                                href={subItem.path}
                                className={`
                                  relative group/item px-4 py-3 text-xs font-mono tracking-wider flex items-center justify-between
                                  hover:bg-white/5 transition-colors
                                  ${isSubActive ? 'text-endfield-accent' : 'text-gray-400 hover:text-white'}
                                `}
                              >
                                <span>{subItem.name}</span>
                                
                                {/* 悬停时的箭头提示 */}
                                <span className="opacity-0 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all duration-300 text-endfield-accent">
                                  &gt;
                                </span>

                                {/* 选中时的左侧指示条 */}
                                {isSubActive && (
                                  <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-endfield-accent" />
                                )}
                              </Link>
                            );
                          })}
