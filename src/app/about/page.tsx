...
export default async function AboutPage() {
  ...
  return (
    <main className="min-h-screen relative text-white selection:bg-endfield-accent selection:text-black">
      ...
      {/* RootLayout 已包含 Navbar，这里不再重复引入，避免双导航栏 */}
      {/* <Navbar /> */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 bg-endfield-surface/50 border border-white/10 p-8 relative backdrop-blur-sm">
           ...
           <div className="lg:col-span-4 flex flex-col gap-6 border-b lg:border-b-0 lg:border-r border-white/10 pb-8 lg:pb-0 lg:pr-8">
              <div className="relative w-full aspect-square border-2 border-white/10 p-2 overflow-hidden">
                 ...
                 <img src={aboutData.avatar} alt="Avatar" className="w-full h-full object-cover" />
                 <div className="absolute bottom-4 right-4 text-[10px] font-mono text-endfield-accent bg-black/80 px-2 py-1 z-20">IMG_SOURCE_RAW</div>
              </div>
                 ...
                 {aboutData.logo && (
                   <div className="mt-2">
                     ...
                     <div className="w-full h-48 border border-white/10 bg-black/40 p-4 flex items-center justify-center">
                       <img src={aboutData.logo} alt="Affiliation Logo" className="w-full h-full object-contain" />
                     </div>
                   </div>
                 )}
              </div>
              ...
              <div className="mt-auto pt-6">
                <a href={`mailto:${aboutData.email}`} className="block w-full text-center bg-endfield-accent text-black font-bold py-3 text-sm hover:bg-white transition-colors clip-path-button uppercase tracking-widest">
                  SEND_TRANSMISSION
                </a>
           ...
           <div className="lg:col-span-8 flex flex-col gap-10">
              <div>
                 ...
                 <div className="text-gray-300 font-mono text-sm leading-loose whitespace-pre-wrap bg-black/40 border border-white/5 p-6 relative">
                    <div className="absolute top-4 right-4 w-20 h-20 opacity-10 pointer-events-none" style={{ backgroundImage: 'repeating-linear-gradient(45deg, #fff 0, #fff 1px, transparent 0, transparent 10px)' }} />
              ...
              <div>
                 ...
                 {/* === 关键修复点：响应式 Grid === */}
                 {/* 
                     grid-cols-1: 手机默认单列，显示效果最好，像列表一样
                     sm:grid-cols-2: 平板/大屏手机变为双列
                     lg:grid-cols-3: 桌面端变为三列
                 */}
